...
const CustomButton: React.FC<CustomButtonProps> = ({
  ...
  return (
    <button
      onClick={handleClick}
      className={cn(
        `px-3 md:px-3 text-xs md:text-sm z-20 mt-2 md:mt-5 h-8 animate-shimmer items-center flex flex-row rounded-xl border border-slate-800 bg-[linear-gradient(110deg,#000103,45%,#1e2631,55%,#000103)] bg-[length:200%_100%]  dark:text-slate-400 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50`,
        className
      )}
    >
      <p className="pr-2 text-white dark:text-neutral-400">{children}</p>
      <svg
        width="16"
        height="16"
        viewBox="0 0 16 16"
