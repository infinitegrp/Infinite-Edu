...
export default function WorkInDubai() {
   return (
      <>
         <div className="h-full w-full justify-center items-start  overflow-hidden flex flex-col relative">
            <motion.div className='pt-4 md:pt-4 lg:pt-6 px-4 md:px-5 lg:px-6  h-full w-full  bg-white dark:bg-neutral-950'>
               <h1 className='font-semibold text-[0.98rem] md:text-xl text-black dark:text-white'>Study and Work in Dubai</h1>
               <p className='text-sm lg:text-base text-gray-500 dark:text-gray-300 pt-2 md:pt-2 lg:pt-3 '>
                  Students looking to pursue their education in the UAE will find
               </p>
               <div className='d-flex flex-row'>
                  <Link href={'/dubai'} >
                     <button className="absolute z-20 mt-3 md:mt-3 h-8 animate-shimmer items-center flex flex-row rounded-xl border border-slate-800 bg-[linear-gradient(110deg,#000103,45%,#1e2631,55%,#000103)] bg-[length:200%_100%] px-3 text-sm dark:text-slate-400 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
